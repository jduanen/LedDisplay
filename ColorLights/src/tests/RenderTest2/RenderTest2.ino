...
void loop() {
	...
	ledDisplayPtr->drawPixel(row, col, color);
    ledDisplayPtr->show();
    ...
    case STRIPES_PAT:
        if (loopCnt & 1) {
            for (col = 0; (col < NUM_COLS); col++) {
                ledDisplayPtr->drawPixel(col, 0, RED_COLOR);
                ledDisplayPtr->drawPixel(col, 4, RED_COLOR);
            }
            for (col = 0; (col < NUM_COLS); col++) {
                ledDisplayPtr->drawPixel(col, 1, GREEN_COLOR);
                ledDisplayPtr->drawPixel(col, 5, GREEN_COLOR);
            }
            for (col = 0; (col < NUM_COLS); col++) {
                ledDisplayPtr->drawPixel(col, 2, AMBER_COLOR);
                ledDisplayPtr->drawPixel(col, 6, AMBER_COLOR);
            }
            for (col = 0; (col < NUM_COLS); col++) {
                ledDisplayPtr->drawPixel(col, 3, BLACK_COLOR);
            }
        } else {
            for (row = 0; (row < NUM_ROWS); row++) {
                for (col = 0; (col < NUM_COLS); col++) {
                    ledDisplayPtr->drawPixel(col, row, BLACK_COLOR);
                }
            }
        }
        break;
